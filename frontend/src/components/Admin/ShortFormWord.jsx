...
export default function ShortFormWord() {
  ...
  const [currentWordId, setCurrentWordId] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [duplicateAlert, setDuplicateAlert] = useState(false); // State for duplicate word alert
  ...
  // Handle Add or Update Word Form submission
  const handleAddOrUpdateWord = async () => {
    // Check for duplicate short form word
    const duplicateWord = words.find(word => word.shortForm.toLowerCase() === newWord.shortForm.toLowerCase());
    
    if (duplicateWord && !currentWordId) {
      // Show alert if duplicate is found and we're adding a new word (not editing)
      setShowAddWordForm(false);
    ...
    try {
      ...
      setCurrentWordId(null);
      setShowAddWordForm(false);
      setDuplicateAlert(false); // Reset alert after successful add/update
    } catch (error) {
      console.error('Error saving short form word:', error);
    }
  };
  ...
  return (
    <div className="container mt-5" style={{ paddingLeft: "0px" }}>
      ...
      {/*{duplicateAlert && (
        <Alert variant="danger" onClose={() => setDuplicateAlert(false)} dismissible>
          A word with this short form already exists!
        </Alert>
      )}*/}
      {duplicateAlert && (
  <Alert 
    ...
    A word with this short form already exists!
            ...
            <tr key={word.shortForm}>
              ...
              <td style={{ padding: "0px", verticalAlign: "middle" }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Button
                    variant="danger"
                    style={{ backgroundColor: '#d9534f', borderColor: '#d43f3a', marginLeft: '2px' }}
                    onClick={() => handleDeleteWord(word.shortForm)}
                  >
                    <FaTrash />
                  </Button>
      ...
      {showAddWordForm && (
        ...
        <MdCancel
          ...
          <Button className="mt-3" variant="primary" onClick={handleAddOrUpdateWord}>
            {currentWordId ? "Update Word" : "Add Word"}
          </Button>
        </Form>
        </div>
